...
public class LireDemoFrame extends javax.swing.JFrame {
	...
	private String getGroup(Document doc) {
		String file = doc.getFieldable(net.semanticmetadata.lire.DocumentBuilder.FIELD_NAME_IDENTIFIER).stringValue();
		return getGroup(file);
	}
	
	private String getGroup(String file) {
		int i = file.indexOf('_');
		int j = file.indexOf('.');		
		String groupName = file.substring(i+1, j);
		...
		Thread t = new Thread() {
			...
			public void run() {
				try {
					...
					String groupName = getGroup(myDoc);
					System.out.println("in searchForDocument and group is " + groupName);
					ImageSearchHits hits = searcher.search(myDoc, reader);
					tableModel.setHits(hits, progressSearch, groupName);
					reader.close();
					// scroll to first row:
	...
	private void rerankFeatureActionPerformed(java.awt.event.ActionEvent evt) throws IOException {//GEN-FIRST:event_rerankFeatureActionPerformed
